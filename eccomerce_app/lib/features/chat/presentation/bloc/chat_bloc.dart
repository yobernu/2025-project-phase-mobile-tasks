...
class ChatBloc extends Bloc<ChatEvent, ChatState> {
  ...
  Future<void> _onLoadChatById(LoadChatById event, Emitter<ChatState> emit) async {
    emit(ChatLoading());
  ...
  Future<void> _onLoadChatMessages(LoadChatMessages event, Emitter<ChatState> emit) async {
    ...
    result.fold(
      (failure) => emit(ChatError(_mapFailureToMessage(failure))),
      (messages) => emit(ChatMessagesLoaded(chatId: event.chatId, messages: messages)),
    );
  }

  Future<void> _onInitiateNewChat(InitiateNewChat event, Emitter<ChatState> emit) async {
    emit(ChatLoading());
  ...
  Future<void> _onConnectToRealTime(ConnectToRealTime event, Emitter<ChatState> emit) async {
    try {
  ...
  Future<void> _onDisconnectFromRealTime(DisconnectFromRealTime event, Emitter<ChatState> emit) async {
    try {
