...
class TestHelpers {
  ...
  static void mockSuccessfulDelete(
    ...
    when(
      mockClient.delete(Uri.parse('$baseUrl$endpoint'), headers: headers),
  ...
  static void mockFailedRequest(
    ...
    switch (method.toUpperCase()) {
      ...
      case 'PUT':
        when(
          mockClient.put(uri, headers: headers, body: anyNamed('body')),
        ).thenAnswer((_) async => http.Response('Error', statusCode));
        break;
      case 'DELETE':
        when(
          mockClient.delete(uri, headers: headers),
        ).thenAnswer((_) async => http.Response('Error', statusCode));
        break;
    }
  }

  static void verifyGetCalled(
    dynamic mockClient,
    String endpoint, {
    int times = 1,
  }) {
  ...
  static void verifyDeleteCalled(
    dynamic mockClient,
    String endpoint, {
    int times = 1,
  }) {
